...
const APPLICATION_ID='application_id.txt'
...
function read_sensor_id_list(){
    ...
    console.log("device_eui ---------------- " + sensor_ids);

    var str = String(fs.readFileSync(APPLICATION_ID));
    app_id = str.split(',');
    for (i = 0; i < app_id.length; i++){
       app_id[i] = JSON.parse(app_id[i]);
       app_id[i] = app_id[i]["app"];
    }
...
function ls_on_mqtt_connect() { //lora_app_server mqtt connet
    for (var i = 0; i < sensor_ids.length; i++){
        var ls_rxtopic = util.format('application/%s/device/%s/event/up', app_id.toString(), sensor_ids[i].toLowerCase());
...
function ls_on_mqtt_message_recv(topic, message) {
    ...
    var dev_eui_en = new Buffer.from (message_parse['devEUI'],'base64'); 
    var dev_eui=dev_eui_en.toString('hex') 
